...
public class ExcelReadUtils {
     ...
     * WORKBOOK模式浮点数格式
     */
    private static final HSSFDataFormatter HDF = new HSSFDataFormatter();
    ...
    public static BigDecimal readBigDecimal(Cell cell, int scale) {
        ...
        switch (ctype) {
            case NUMERIC:
                d2 = BigDecimal.valueOf(cell.getNumericCellValue());
                break;
            case STRING:
                try {
                ...
                break;
            case BOOLEAN:
                d2 = BigDecimal.valueOf(cell.getBooleanCellValue() ? 1d : 0d);
                break;
            case FORMULA:
                FormulaEvaluator evaluator = cell.getSheet().getWorkbook().getCreationHelper().createFormulaEvaluator();
                CellValue value = evaluator.evaluate(cell);
                ...
                switch (value.getCellTypeEnum()) {
                    case NUMERIC:
                        d2 = BigDecimal.valueOf(value.getNumberValue());
                        break;
                    case STRING:
                        try {
                        ...
                        break;
                    case BOOLEAN:
                        d2 = BigDecimal.valueOf(value.getBooleanValue() ? 1d : 0d);
                        break;
                    default:
                        break;
                }
